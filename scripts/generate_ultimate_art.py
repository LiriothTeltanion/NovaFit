...
def build_banner(output: Path) -> Path:
    """Generate the primary Ultimate 4.0 hero artwork."""
    cards = [
        (90, 355, "3 LOCAL PROFILES", "Isolated dates, goals & themes", "card"),
        (435, 355, "EN · ES · HE", "LTR + true Hebrew RTL shell", "card2"),
        (780, 355, "12 THEMES", "Dark, light & high contrast", "card"),
        (1125, 355, "SPORT + DATA COACH", "Conservative, explainable actions", "card2"),
    ]
    card_markup = "".join(
...
def build_profiles(output: Path) -> Path:
    ...
    <g class="float"><rect x="70" y="155" width="280" height="230" rx="28" class="card"/><circle cx="140" cy="225" r="38" fill="#2dd4bf"/><text x="200" y="216" class="label">Kevin / Lirioth</text><text x="200" y="246" class="small">English · Aurora · Mixed</text><text x="105" y="315" class="subtitle">84 private records</text></g>
    <g class="float delay1"><rect x="405" y="155" width="280" height="230" rx="28" class="card2"/><circle cx="475" cy="225" r="38" fill="#fb7185"/><text x="535" y="216" class="label">Lucía Demo</text><text x="535" y="246" class="small">Español · Arcade · Walking</text><text x="440" y="315" class="subtitle">23 isolated records</text></g>
    <g class="float delay2"><rect x="740" y="155" width="280" height="230" rx="28" class="card"/><circle cx="810" cy="225" r="38" fill="#60a5fa"/><text x="970" y="216" class="label" text-anchor="end">נועה Demo</text><text x="970" y="246" class="small" text-anchor="end">עברית · Sapphire · Mobility</text><text x="775" y="315" class="subtitle">20 רשומות מבודדות</text></g>
...
def build_recommendations(output: Path) -> Path:
    ...
    arrows = ''.join(f'<path d="M{x} 255H{x+55}" class="line dash"/><path d="M{x+45} 243l20 12-20 12" fill="#2dd4bf"/>' for x in (330,635,940))
...
def build_atlas(output: Path) -> Path:
    ...
    <g transform="translate(890 175)"><rect width="440" height="235" rx="25" class="card2"/><g fill="#60a5fa">{''.join(f'<rect x="{30+i*48}" y="{190-(i%5)*24}" width="26" height="{35+(i%5)*24}" rx="7" opacity="{.55+i*.04}"/>' for i in range(8))}</g><path d="M28 140C100 95 165 160 230 105S355 60 410 92" class="line2"/><text x="28" y="35" class="label">Hydration + weekday rhythm</text><text x="28" y="218" class="small">Coach lens: progress gradually · protect recovery · improve data consistency</text></g>'''
...
def build_technology(output: Path) -> Path:
    ...
    labels = ["Python 3.10+", "Tkinter / ttk", "SQLite v4", "Matplotlib", "Pillow icons", "unittest", "JSON / CSV", "Windows BAT", "GitHub Actions"]
    nodes = []
    import math
    for i, label in enumerate(labels):
        ...
        nodes.append(f'<g class="float delay{i%4}"><circle cx="{x:.0f}" cy="{y:.0f}" r="57" fill="#0b2834" stroke="{"#2dd4bf" if i%2==0 else "#60a5fa"}" stroke-width="2"/><text x="{x:.0f}" y="{y+5:.0f}" class="small" text-anchor="middle">{label}</text></g><path d="M700 270L{x:.0f} {y:.0f}" class="line" opacity=".28"/>')
    body = f'''<rect width="1400" height="520" class="bg"/>{grid(1400,520,70)}<text x="55" y="62" class="title">Technology constellation</text><text x="55" y="96" class="subtitle">Purposeful dependencies around one local-first Python core.</text>{''.join(nodes)}<circle cx="700" cy="270" r="96" fill="url(#aurora)" opacity=".19" class="pulse"/><circle cx="700" cy="270" r="68" fill="#071f2d" stroke="#c084fc" stroke-width="3"/><text x="700" y="263" class="label" text-anchor="middle">NOVAFIT</text><text x="700" y="292" class="small" text-anchor="middle">ULTIMATE 4.0</text>'''
...
def build_distribution(output: Path) -> Path:
    ...
    <rect x="350" y="370" width="700" height="42" rx="18" fill="#122a32" stroke="#34d399"/><text x="700" y="397" class="label" text-anchor="middle">No more false failure after 71/71 tests: personal data is preserved, releases stay clean.</text>'''
...
def build_icon_sheet(output: Path) -> Path:
    ...
    names = ["dashboard", "motivation", "recommendations", "add", "records", "profiles", "tools", "language", "theme"]
    palette = {"accent": "#2dd4bf", "accent_alt": "#c084fc", "panel_alt": "#0c3238", "border": "#266274"}
    ...
    for i, name in enumerate(names):
        x = 38 + i * 135
        y = 88
        draw.rounded_rectangle((x, y, x + 104, y + 104), radius=24, fill=palette["panel_alt"], outline=palette["border"], width=3)
        # Symbols are rendered on dedicated transparent tiles below.
    # Redraw symbols correctly on dedicated tiles to preserve the same implementation geometry.
    for i, name in enumerate(names):
        tile = Image.new("RGBA", (104, 104), (0, 0, 0, 0))
        tile_draw = ImageDraw.Draw(tile)
        IconFactory._draw_symbol(tile_draw, name, 104, palette["accent"], palette["accent_alt"], 4)
        x = 38 + i * 135
        sheet.alpha_composite(tile, (x, 88))
        label_box = draw.textbbox((0, 0), name, font=font)
        label_w = label_box[2] - label_box[0]
        draw.text((x + 52 - label_w / 2, 215), name, fill="#b7dbe3", font=font)
    draw.text((38, 274), "Decorative icons are always paired with readable text labels and adapt to every theme.", fill="#8db6c2", font=font)
