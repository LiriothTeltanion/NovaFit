...
class TrainingAtlasTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.entries = [
            HealthEntry.build(f"2026-07-{day:02d}", 6000 + day * 300, 1.4 + day * 0.05, mood="Focused")
    ...
    def test_training_atlas_has_five_axes(self) -> None:
        figure = create_dashboard_figure(
            self.entries, AppSettings(), view="training_atlas", theme="sapphire", days=14
        )
    ...
    def test_training_atlas_exports_png(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            save_dashboard_chart(
                self.entries, AppSettings(), path, view="training_atlas", theme="arcade", days=14
            )
